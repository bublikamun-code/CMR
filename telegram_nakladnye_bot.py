            ...
            messages=[
                {
                    ...
                    "content": (
                        ...
                        "На фото: ТН (товарная накладная), ТТН (товарно-транспортная накладная) "
                        "или УПД (универсальный передаточный документ).\n\n"
                        "ВАЖНО: компания «Свет в доме» / «ООО Свет в доме» — это НАША компания (покупатель). "
                        "В качестве supplier_name укажи ДРУГУЮ сторону — контрагента (поставщика/продавца). "
                        "Если накладная на возврат — поставщик это грузополучатель.\n\n"
                        "Извлеки поля и верни ТОЛЬКО валидный JSON:\n"
                        "{\n"
                        '  "supplier_name": "название контрагента (НЕ «Свет в доме») — '
                        'ИП, ООО, ОДО, ЧТУП и т.д.",\n'
                        '  "doc_type": "ТН" или "ТТН" или "УПД",\n'
                        '  "doc_series": "серия бланка (буквы перед номером, например ЯЖ, АВ, МК)",\n'
                        '  "doc_number": "номер накладной — только цифры",\n'
                        '  "doc_date": "дата в формате YYYY-MM-DD",\n'
                        '  "amount": число — ИТОГО С НДС (строка "Всего стоимость с НДС" или "Итого с НДС" '
                        'в нижней части документа. Это число, а не текст прописью!)\n'
                        '  "vat_amount": число — СУММА НДС (строка "Всего сумма НДС" или "Итого НДС")\n'
                        '  "unload_address": "адрес разгрузки (для ТТН)"\n'
                        "}\n"
                        "Если поле не распознано — null.\n"
                        "ВАЖНО: amount и vat_amount — это ЧИСЛА (например 2170.80), а не текст.\n"
                        "Бери данные из ИТОГОВОЙ строки таблицы, а не из промежуточных строк.\n"
