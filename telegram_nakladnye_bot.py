...
OCR_PROMPT = (
    ...
    "• has_second_page — true если:\n"
    "  - таблица товаров обрывается (нет строки «Итого» внизу этой страницы)\n"
    "  - есть текст «продолжение на обороте» / «страница 1 из 2»\n"
    "  - видно что это только первая страница документа\n"
    "  - ВНИМАНИЕ: если на странице есть товары БЕЗ итоговой строки «Итого» — "
    "значит есть 2-я страница! Ставь has_second_page: true.\n"
    "  - Если на странице подписи/печати без товаров — has_second_page: false.\n\n"
    "СУММЫ — КРИТИЧНО:\n"
...
async def ocr_photos_batch(images_bytes: list):
    ...
    if not result:
        logger.warning("Both batch models failed, falling back to per-photo OCR (Qwen3.8 Flash)")
        result = []
        for i, img_bytes in enumerate(images_bytes):
            b64 = base64.b64encode(compress_image(img_bytes)).decode()
            single_content = [
                {"type": "text", "text": "Распознай накладную на этом фото. Верни массив из одного элемента."},
                {"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{b64}"}},
            ]
            # Qwen для одиночных фото (лучшее качество), fallback Gemini
            single_result = await loop.run_in_executor(None, _ocr_call, OCR_SINGLE, single_content)
            ...
            if single_result:
                ...
                logger.error(f"Per-photo OCR failed for photo {i}")
