...
### 6.1. Решение по V7 и V11 — пункт 17 плана закрыт (добавлено 23.09)

**V7 — ЗАКРЫТО без новой колонки.** `transactions.is_invoice_doc` объявлен
каноническим хранилищем ОДНОГО бизнес-факта «оригинал ТН у нас», у которого три
подписи в интерфейсах. Конфликт был конфликт подписей, а не данных:

| Подпись | Где | Источник данных |
| --- | --- | --- |
| «Оригинал ТН возвращён» | карточка сделки v2, `mockups/shell-v2-board.js:918` (запись `:1902-1921`) | `docs[].originalsReturned` ← `is_invoice_doc` записи-документа (`v2-boot-template.js:271`) |
| «ТН у нас» | финансы v2, `site-v2/js/shell-v2-fin.js:50,198,418` | тот же `is_invoice_doc` (`v2-boot-template.js:559`, запись `:757-760`) |
| колонка «ТН» (title «Накладная (ТН)») | «Документы» legacy, `site/index.html:299`, `site/js/documents.js:185,282` | тот же `is_invoice_doc` |

Все три экрана читают и пишут одну колонку одной и той же записи-документа
(`is_document=True`), а v2 дополнительно синхронизирует карточку и финансы
событием `fin:originals` (`shell-v2-fin.js:422-423`). В кодировке проекта
«invoice» = накладная (`invoice_number` — № ТН, `issue_invoice` — выписать
накладную), поэтому имя колонки читается как «накладная-документ у нас» и
смыслу не противоречит. `is_bill_doc` — парный факт «оригинал счёта у нас».

Почему НЕ колонка `originals_returned`: боевые данные уже заполнены в этом
смысле через «Документы», перенос дал бы два источника правды на один факт,
а клиентская часть (board.js/boot.js) сейчас занята другими пунктами плана.

Что закреплено сервером (правки 23.09):

- `services/payments.py:47-55` — семантика пары флагов задокументирована в
  `tx_dict` (единственное место сериализации реестра);
- `routers/payments_router.py:320-336` — `PATCH /payments/transactions/{id}`
  отклоняет `is_invoice_doc`/`is_bill_doc` на строке, которая НЕ является
  документом (400 с внятной причиной). Раньше такой PATCH отвечал 200, флаг
  оседал на строке реестра оплат и не читался ни одним экраном — то есть
  терялся молча. Оба клиента (v2 и legacy) пишут флаг только в записи-документы
  (`v2-boot-template.js:760`, `site/js/documents.js:282`), поэтому действующих
  сценариев гейт не ломает;
- `tests/test_originals_returned_v7.py` — контракт: свежая копия-документ
  создаётся без флага (выписка ≠ «оригинал у нас»), флаг круговой
  (PATCH → `GET /payments/documents`), на строке реестра — 400, остальные поля
  строки реестра правятся как раньше, документ групповой ТН флаг принимает,
  отмена накладной убирает документ вместе с флагом.

Остаток по V7 — клиентский (не серверный, в скоупе пунктов 14/16):

1. Групповые ТН в финансах v2: строка группы собирается без `docTxId` и с
   жёстким `tnHere: false` (`site-v2/js/shell-v2-fin.js:218`), поэтому галочка
   «ТН у нас» у групповой накладной не сохраняется. Сервер флаг принимает
   (тест `test_group_document_accepts_originals_flag`); нужно в
   `tools/v2-boot-template.js` отдавать `docTxId` документа группы
   (`writeoff_group_id` + `is_document=True`) и `tnHere` из его `is_invoice_doc`.
2. Подпись «Оригинал ТН возвращён» в карточке и «ТН у нас» в финансах стоит
   привести к одной формулировке (например «Оригинал ТН у нас») — иначе у
   пользователя сохраняется впечатление двух разных фактов. Правка в
   `mockups/shell-v2-board.js:918` (файл занят пунктом 3) и в inline-скрипте
   финансов `mockups/shell-v2-prototype.html` (источник `shell-v2-fin.js`,
   занят пунктом 3) — поэтому руками не вносилась.

**V11 — ЗАКРЫТО.** Матрица ролей на деструктивных бизнес-операциях
(`require_role("admin", "superadmin")`, супер-админ наследует админа):

| Операция | Роут | Было | Стало | Где |
| --- | --- | --- | --- | --- |
| Отмена накладной/оплаты (удаление записи реестра; тем же роутом — отмена групповой ТН) | `DELETE /payments/transactions/{id}` | любой аутентифицированный | admin | `routers/payments_router.py:296-304` |
| Отмена списания сделки целиком (сносит все записи и копии) | `DELETE /payments/cards/{id}/writeoff` | любой аутентифицированный | admin | `routers/payments_router.py:594-595` |
| Удаление поставщика (справочник) | `DELETE /suppliers/{id}` | любой аутентифицированный | admin | `routers/suppliers_router.py:103-108` |
| Удаление тега (справочник; каскадом снимается со всех сделок) | `DELETE /tags/{id}` | любой аутентифицированный | admin | `routers/tags_router.py:45-51` |
| Удаление входящей накладной (стирает и фото с диска) | `DELETE /nakladnye/{id}` | любой аутентифицированный | admin | `routers/nakladnye_router.py:179-185` |
| Удаление клиента / прихода клиента / сделки навсегда / починка списаний / пользователи / справочники / вебхуки / воркфлоу / кастомные объекты | `DELETE /clients/{id}`, `DELETE /clients/payments/{id}`, `DELETE /kanban/cards/{id}/permanent`, `POST /payments/repair-writeoffs`, `/auth/users`, `/dictionaries/*`, `/webhooks`, `/workflows`, `/custom` | admin | без изменений (регрессия в тестах) | — |

Операционные действия гейты НЕ трогают (проверено тестами): перенос сделки в
реестр, выписка накладной и групповой ТН, галочки реестра (`PATCH`), копия в
«Документы», мягкое удаление сделки в корзину и восстановление, приём и правка
входящих накладных, теги на сделке, группы списания, `finish_assembly` и
`execute` у склада. Проверки владельца остались там, где уже были: задачи
(автор или админ), комментарии ленты (автор или админ), уведомления (фильтр по
`user_id`).

Сознательно НЕ закрыто: менеджер по-прежнему правит суммы чужой сделки
(проверки владельца нет, `test_access_matrix.py`). «Менеджер — только свои
операционные действия» в полном объёме требует отдельного решения владельца:
карточки в CRM правят и склад, и документооборот, поэтому ограничение по
`owner_id` пришлось бы вводить сразу на всех экранах, а не на удалении.

Требует подтверждения владельцем (поведение прода изменится):

- роли `warehouse` и `documents` больше не удаляют записи реестра и не отменяют
  списание сделки — только админ. Если складу нужна самостоятельная отмена
  списания, достаточно снять гейт с `DELETE /payments/cards/{id}/writeoff`
  (ручку зовёт только legacy: `site/js/writeoffs.js:401`, `site/js/payments.js:297`;
  v2 её не использует) либо расширить список ролей в одном месте;
- `DELETE /nakladnye/{id}` закрыт, хотя ни один фронт его не зовёт (grep по
  `mockups/`, шаблонам v2 и `site/js` — 0 обращений): ошибочную накладную от
  бота теперь убирает админ.

Тесты: `tests/test_role_gates_v11.py` (матрица таблицей в docstring, 403 для
manager/warehouse/documents и успех для admin/superadmin на каждый закрытый
роут + операционные контроли). Три теста, которые раньше фиксировали
несогласованность как «текущее поведение», переписаны под новую матрицу
(`test_access_matrix.py`, `test_data_integrity.py`, `test_nakladnye_dedup.py`);
бизнес-тесты удалений переведены на админа (`test_payments_money.py`,
`test_statuses_kanban.py`).

**Попутная находка (не пункт 17, решение за владельцем):**
`POST /email-parser/settings` (`routers/email_parser_router.py:274-296`) доступен
ролям manager и warehouse (роутерный `require_role` на `:102` не пускает только
`documents`). Ручка перезаписывает `email`, `password` и `imap_server` почтового
ящика CRM, то есть пользователь может увести разбор входящей почты на свой
сервер. Рекомендуется `require_admin()` — как у остальных настроек
(`/dictionaries`, `/webhooks`, `/workflows`).
