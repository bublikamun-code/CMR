"""Тесты миграции 0015 (ротация record_versions и activity_log).
...
MIGRATION = "0015_rotate_history_tables.py"
...
def test_0015_keeps_newest_versions_per_record():
    conn = _db()
    cur = conn.cursor()
...
def test_0015_caps_each_record_independently():
    conn = _db()
    cur = conn.cursor()
...
def test_0015_records_under_cap_untouched():
    conn = _db()
    cur = conn.cursor()
...
def test_0015_fresh_versions_are_never_deleted():
...
def test_0015_versions_without_timestamp_are_kept():
...
def test_0015_mixed_timestamp_formats_compared_correctly():
...
def test_0015_deletes_log_older_than_retention_horizon():
    conn = _db()
    cur = conn.cursor()
...
def test_0015_keeps_log_within_horizon_even_across_many_cards():
    conn = _db()
    cur = conn.cursor()
    for card in (1, 2, 3):
...
def test_0015_caps_log_per_card_keeping_newest():
    conn = _db()
    cur = conn.cursor()
    cur.execute("INSERT INTO cards (id, title) VALUES (1, 'Сделка')")
...
def test_0015_per_card_cap_does_not_apply_to_null_card():
...
def test_0015_comments_are_never_deleted():
...
def test_0015_fresh_log_is_never_deleted():
    conn = _db()
    cur = conn.cursor()
    cur.execute("INSERT INTO cards (id, title) VALUES (1, 'Сделка')")
...
def test_0015_log_without_timestamp_is_kept():
    conn = _db()
    cur = conn.cursor()
    cur.execute("INSERT INTO cards (id, title) VALUES (1, 'Сделка')")
...
def test_0015_on_empty_tables_is_noop():
    conn = _db()
...
def test_0015_leaves_foreign_keys_clean():
...
def test_0015_removes_bulk_of_audited_versions_on_real_shape():
