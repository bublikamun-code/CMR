/* ============================================================
   ...
   B14 (пакет E): демо-данные здесь ПОЛНОСТЬЮ АВТОНОМНЫ — собственные
   клиенты, сделки, задачи и справочники с фиксированными ID. Проблема
   демо-режима (?demo=1) в том, что boot.js (строки 779-787) достраивает
   демо-набор реальными данными с сервера (настоящие клиенты попадают в
   демо). Для полного фикса пакет C должен в boot.js при demo=1 ПРОПУСКАТЬ
   загрузку реальных коллекций (results[0..6]) и не вызывать
   buildFinSource/buildKbData — тогда демо будет полностью изолировано.
   ============================================================ */
(function () {
    'use strict';
