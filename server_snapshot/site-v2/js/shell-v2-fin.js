(function () {
    ...
    const finSource = window.KB_FIN_SOURCE || {
        ...
        { supplier: 'ООО «ЭлектроСнаб»', number: 'ВХ-498', date: '15.09.2026', store: 'Богдановича', amount: 180000, vat: 30000, checked: false, arrived: true, paid: false, file: 'electro-498.pdf' },
        { supplier: 'ООО «Люмен»', number: 'ВХ-490', date: '12.09.2026', store: 'БН', amount: 96000, vat: 16000, checked: true, arrived: true, paid: false, file: 'lumen-490.pdf' }
        ]
    };
    ...
    // Месяцы выписки ТН — для фильтра документов
    const docMonths = [...new Set(issued.map((r) => r.tn.date).filter((d) => /^\d{2}\.\d{2}\.\d{4}$/.test(d)).map((d) => d.slice(6, 10) + '-' + d.slice(3, 5)))].sort().reverse();
    ...
    if (docMonthSel) {
        ...
        docMonthSel.innerHTML = '<option value="all">Все месяцы</option>' + docMonths.map((ym) => {
            const parts = ym.split('-');
            return `<option value="${ym}">${names[Number(parts[1]) - 1]} ${parts[0]}</option>`;
        }).join('');
        docMonthSel.value = docMonth;
        if (docMonth !== 'all' && !docMonths.includes(docMonth)) docMonth = 'all';
        docMonthSel.value = docMonth;
    }
